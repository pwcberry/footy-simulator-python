...
class TestMatrixUtilProbFunction(unittest.TestCase):

    def test_returns_base_value_with_zero_inputs(self):
        strength = 0
        accuracy = 0
        pressure = 0
        value = matrix.util.prob(0.2, strength, accuracy, pressure)
...
class TestMatrixUtilNormaliseFunction(unittest.TestCase):
    ...
    def test_vector_is_changed_when_sum_is_greater_than_one_with_dynamic_index(self):
        ...
        dynamic_indexes = [3, 4]
        result = matrix.util.normalise(vector, dynamic_indexes)
